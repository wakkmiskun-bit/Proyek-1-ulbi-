...
<head>
    ...
    <style>
        ...
        .submit-btn { position: relative; overflow: hidden; transition: all 0.4s cubic-bezier(0.34, 1.56, 0.64, 1); animation: slideUp 0.6s cubic-bezier(0.34, 1.56, 0.64, 1) 0.7s both; }
        ...
        .login-link { animation: slideUp 0.6s cubic-bezier(0.34, 1.56, 0.64, 1) 0.8s both; }
    ...
    <div class="relative z-10 min-h-screen flex items-center justify-center px-4 py-10">
        <div class="w-full max-w-md">
            <div class="text-center mb-8">
                ...
                <h2 class="text-2xl font-bold text-white mt-2 md:text-xl">Buat akun baru</h2>
                <p class="text-sm mt-1 md:text-xs" style="color: #8b97b0;">Bergabung dan mulai kelola tugasmu sekarang</p>
            </div>
            <div class="rounded-3xl p-8 card-container md:p-6" style="background: rgba(17,24,39,0.85); border: 1px solid rgba(255,255,255,0.09); backdrop-filter: blur(24px);">
                <form method="POST" action="{{ route('register') }}" class="space-y-5">
                    ...
                    <div class="form-group">
                        <label for="nim" class="block text-xs font-bold uppercase tracking-widest mb-2" style="color: #8b97b0;">NIM</label>
                        <div class="relative">
                            <i class="ti ti-id absolute left-3.5 top-1/2 -translate-y-1/2 text-lg" style="color: #5a6880;"></i>
                            <input id="nim" type="text" name="nim" value="{{ old('nim') }}" required autofocus autocomplete="off" placeholder="Contoh: 1234567890" class="form-input w-full pl-10 pr-4 py-3 rounded-xl text-sm font-medium outline-none md:py-2.5 md:text-xs" style="background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); color: #e8edf5;" onfocus="this.style.borderColor='rgba(99,102,241,0.6)'; this.style.background='rgba(99,102,241,0.08)';" onblur="this.style.borderColor='rgba(255,255,255,0.1)'; this.style.background='rgba(255,255,255,0.05)';">
                        </div>
                        @error('nim') <p class="text-xs mt-1.5 flex items-center gap-1" style="color: #f87171;"><i class="ti ti-alert-circle text-sm"></i> {{ $message }}</p> @enderror
                    </div>
                    <div class="form-group">
                        <label for="name" class="block text-xs font-bold uppercase tracking-widest mb-2" style="color: #8b97b0;">Nama Lengkap</label>
                        <div class="relative">
                            <i class="ti ti-user absolute left-3.5 top-1/2 -translate-y-1/2 text-lg" style="color: #5a6880;"></i>
                            <input id="name" type="text" name="name" value="{{ old('name') }}" required autocomplete="name" placeholder="Nama kamu" class="form-input w-full pl-10 pr-4 py-3 rounded-xl text-sm font-medium outline-none md:py-2.5 md:text-xs" style="background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); color: #e8edf5;" onfocus="this.style.borderColor='rgba(99,102,241,0.6)'; this.style.background='rgba(99,102,241,0.08)';" onblur="this.style.borderColor='rgba(255,255,255,0.1)'; this.style.background='rgba(255,255,255,0.05)';">
                        </div>
                        @error('name') <p class="text-xs mt-1.5 flex items-center gap-1" style="color: #f87171;"><i class="ti ti-alert-circle text-sm"></i> {{ $message }}</p> @enderror
                    </div>
                    <div class="form-group">
                        <label for="email" class="block text-xs font-bold uppercase tracking-widest mb-2" style="color: #8b97b0;">Email</label>
                        <div class="relative">
                            <i class="ti ti-mail absolute left-3.5 top-1/2 -translate-y-1/2 text-lg" style="color: #5a6880;"></i>
                            <input id="email" type="email" name="email" value="{{ old('email') }}" required autocomplete="username" placeholder="email@kamu.com" class="form-input w-full pl-10 pr-4 py-3 rounded-xl text-sm font-medium outline-none md:py-2.5 md:text-xs" style="background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); color: #e8edf5;" onfocus="this.style.borderColor='rgba(99,102,241,0.6)'; this.style.background='rgba(99,102,241,0.08)';" onblur="this.style.borderColor='rgba(255,255,255,0.1)'; this.style.background='rgba(255,255,255,0.05)';">
                        </div>
                        @error('email') <p class="text-xs mt-1.5 flex items-center gap-1" style="color: #f87171;"><i class="ti ti-alert-circle text-sm"></i> {{ $message }}</p> @enderror
                    </div>
                    <div class="form-group">
                        <label for="password" class="block text-xs font-bold uppercase tracking-widest mb-2" style="color: #8b97b0;">Password</label>
                        <div class="relative">
                    ...
                    <div class="form-group">
                        <label for="password_confirmation" class="block text-xs font-bold uppercase tracking-widest mb-2" style="color: #8b97b0;">Konfirmasi Password</label>
                        <div class="relative">
                            <i class="ti ti-lock-check absolute left-3.5 top-1/2 -translate-y-1/2 text-lg" style="color: #5a6880;"></i>
                            <input id="password_confirmation" type="password" name="password_confirmation" required autocomplete="new-password" placeholder="Ulangi password" class="form-input w-full pl-10 pr-10 py-3 rounded-xl text-sm font-medium outline-none md:py-2.5 md:text-xs" style="background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); color: #e8edf5;" onfocus="this.style.borderColor='rgba(99,102,241,0.6)'; this.style.background='rgba(99,102,241,0.08)';" onblur="this.style.borderColor='rgba(255,255,255,0.1)'; this.style.background='rgba(255,255,255,0.05)';">
                            <button type="button" onclick="togglePwd('password_confirmation', 'eye-icon-2')" class="absolute right-3.5 top-1/2 -translate-y-1/2 transition-colors hover:text-white" style="color: #5a6880; background: none; border: none; cursor: pointer;"><i id="eye-icon-2" class="ti ti-eye text-lg"></i></button>
                        </div>
                        @error('password_confirmation') <p class="text-xs mt-1.5 flex items-center gap-1" style="color: #f87171;"><i class="ti ti-alert-circle text-sm"></i> {{ $message }}</p> @enderror
                    </div>
                    <button type="submit" class="submit-btn w-full py-3.5 rounded-xl font-bold text-base text-white flex items-center justify-center gap-2 md:py-3 md:text-sm" style="background: linear-gradient(135deg, #6366f1 0%, #8b5cf6 50%, #d946ef 100%); background-size: 200% 200%; box-shadow: 0 4px 24px rgba(99,102,241,0.4); animation: gradientShift 3s ease infinite;">
                        <i class="ti ti-arrow-right text-lg md:text-base"></i><span>Lanjut Daftar</span>
                    </button>
                </form>
            </div>
            <p class="text-center text-sm mt-6 login-link md:text-xs" style="color: #5a6880;">Sudah punya akun? <a href="{{ route('login') }}" class="font-bold transition-colors hover:text-indigo-300" style="color: #a5b4fc;">Login di sini</a></p>
        </div>
    </div>
    <script>
