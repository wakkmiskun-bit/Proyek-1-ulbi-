    ...
    <div class="relative z-10 min-h-screen flex items-center justify-center px-4 py-10">
        <div class="w-full max-w-md">
            <div class="text-center mb-8">
                ...
                <h2 class="brand-name text-2xl font-bold mt-2" style="color: #3d2b27;">Selamat datang kembali</h2>
                <p class="text-sm mt-1" style="color: #705953;">Masuk ke akun TaskMate kamu</p>
            </div>
            @if (session('status'))
            ...
            <div class="rounded-3xl p-8 card-container" style="background: #ffffff; border: 1px solid rgba(61,43,39,0.08); box-shadow: 0 16px 48px rgba(61,43,39,0.06);">
                <form method="POST" action="{{ route('login') }}" class="space-y-5">
                    ...
                    <div class="form-group">
                        <label for="login" class="block text-xs font-bold uppercase tracking-widest mb-2" style="color: #705953;">NIM atau Email</label>
                        <div class="relative">
                            <i class="ti ti-id absolute left-3.5 top-1/2 -translate-y-1/2 text-lg" style="color: #a8938e;"></i>
                            <input id="login" type="text" name="login" value="{{ old('login') }}" required autofocus autocomplete="username" placeholder="NIM atau email@kamu.com" class="form-input w-full pl-10 pr-4 py-3 rounded-xl text-sm font-medium outline-none" style="background: #fffdfa; border: 1px solid rgba(61,43,39,0.15); color: #3d2b27;">
                        </div>
                        @error('login') <p class="text-xs mt-1.5 flex items-center gap-1" style="color: #ef4444;"><i class="ti ti-alert-circle text-sm"></i> {{ $message }}</p> @enderror
                    </div>
                    <div class="form-group">
                        <div class="flex items-center justify-between mb-2">
                            <label for="password" class="text-xs font-bold uppercase tracking-widest" style="color: #705953;">Password</label>
                            <a href="{{ route('bantuan') }}" class="text-xs font-semibold" style="color: #e91e63;">Bantuan</a>
                        </div>
                        <div class="relative">
                            <i class="ti ti-lock absolute left-3.5 top-1/2 -translate-y-1/2 text-lg" style="color: #a8938e;"></i>
                            <input id="password" type="password" name="password" required autocomplete="current-password" placeholder="Password kamu" class="form-input w-full pl-10 pr-10 py-3 rounded-xl text-sm font-medium outline-none" style="background: #fffdfa; border: 1px solid rgba(61,43,39,0.15); color: #3d2b27;">
                    ...
                    <button type="submit" class="submit-btn w-full py-3.5 rounded-xl font-bold text-base text-white flex items-center justify-center gap-2" style="background: linear-gradient(135deg, #e91e63 0%, #f06292 50%, #ec407a 100%); background-size: 200% 200%; box-shadow: 0 4px 24px rgba(233, 30, 99, 0.35); animation: gradientShift 3s ease infinite;">
                        <i class="ti ti-arrow-right text-lg"></i><span>Masuk Sekarang</span>
                    </button>
                </form>
            </div>
            <p class="text-center text-sm mt-6 login-link" style="color: #705953;">Belum punya akun? <a href="{{ route('register') }}" class="font-bold" style="color: #e91e63;">Daftar sekarang</a></p>
            <p class="text-center text-xs mt-3 login-link" style="color: #a8938e;"><a href="{{ route('admin.login') }}" style="color: #a8938e;">Login Admin</a></p>
        </div>
    </div>
    <script>
